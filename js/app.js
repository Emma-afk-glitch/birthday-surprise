...
(() => {
  ...
  const QUESTIONS = {
    boy: [
      { emoji: '👨', text: 'Are you the favorite twin?' },
      { emoji: '🏃', text: 'Would you win in a race?' },
      { emoji: '🤣', text: 'Are you the funniest twin?' },
    ],
    girl: [
      { emoji: '👸', text: 'Are you the favorite twin?' },
      { emoji: '🏃', text: 'Would you win in a race?' },
  ...
  function startTheme(theme) {
    ...
    // Start themed ambient background music
    setTimeout(() => {
      if (theme === 'boy') {
        SoundEngine.playBoyAmbient();
      } else {
        SoundEngine.playGirlAmbient();
      }
    }, 400); // slight delay so whoosh plays first
  ...
  // ─────────── REVEAL SCREEN ───────────
  function showReveal() {
    // Stop themed ambient music, transition to birthday!
    SoundEngine.stopThemeMusic();
    SoundEngine.playWhoosh();
    ...
    // 🎶 Fanfare then themed Happy Birthday music!
    SoundEngine.playFanfare();
    setTimeout(() => {
      if (currentTheme === 'boy') {
        SoundEngine.playHappyBirthdayBoy();
      } else {
        SoundEngine.playHappyBirthdayGirl();
      }
    }, 700);
  }
